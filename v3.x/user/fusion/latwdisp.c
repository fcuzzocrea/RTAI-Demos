...

{
	...
	smpl->minjitter = minjitter = rt_timer_ticks2ns(minj);
	smpl->maxjitter = maxjitter = rt_timer_ticks2ns(maxj);
	smpl->avgjitter = avgjitter = rt_timer_ticks2ns(sumj / nsamples);
...

{
    ...
    pause();
    rt_task_join("display");
    rt_task_join("sampling");
    rt_timer_stop();
