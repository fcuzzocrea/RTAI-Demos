...
static inline void rt_start_unix_server(void *task, int rt_prio, int shmsize)
{
	...
	if (!fork()) {
		execl("./syserver", "syserver", args, args + 11, args + 22, args + 33, NULL);
	} 
	shm = (void *)rt_shm_alloc((nam2num("UM") << 16) | pid, shmsize + sizeof(RT_TASK *), SUPRT);
...
static inline int rt_end_unix_server(void)
{
	...
	rt_shm_free((nam2num("UM") << 16) | getpid());
	return ret;
}
